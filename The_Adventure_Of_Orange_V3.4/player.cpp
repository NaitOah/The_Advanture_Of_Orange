...
void move_player(Character *player, int dx, int dy, FloorState &floor1State, FloorState &floor2State, int &currentFloor,
                 ...
                 PIMAGE npc1Image, PIMAGE upStairsImage, PIMAGE downStairsImage) {
    ...
    if (currentFloorState.map[new_x][new_y] != WALL) {
        ...
        if (currentFloorState.map[new_x][new_y] == NPC_1) {
            trigger_dialogue(player, currentFloorState.map, npc1Image, playerImage, playerWithSwordImage, swordImage, orangeImage, enemyImage, enemy2Image, goldImage, wallImage, doorImage, keyImage, heartImage, upStairsImage);
        }
        // 检测是否碰到敌人
        if ((new_x == currentFloorState.enemy.x && new_y == currentFloorState.enemy.y) ||
            (new_x == currentFloorState.enemy2.x && new_y == currentFloorState.enemy2.y)) {
            Character *enemy = (new_x == currentFloorState.enemy.x && new_y == currentFloorState.enemy.y) ? &currentFloorState.enemy : &currentFloorState.enemy2;
            if (enemy->hp > 0) { // 只有在敌人活着的时候才进行战斗
                while (player->hp > 0 && enemy->hp > 0) {
                    ...
                    for (int i = 0; i < 5; i++) {
                        cleardevice();
                        Sleep(50);
                        draw_map(currentFloorState.map, *player, currentFloorState.enemy, currentFloorState.enemy2, playerImage, playerWithSwordImage, swordImage, orangeImage, enemyImage, enemy2Image, goldImage, wallImage, doorImage, keyImage, npc1Image, upStairsImage, downStairsImage, true);
                ...
                if (enemy->hp <= 0) {
                    ...
                    currentFloorState.map[enemy->x][enemy->y] = EMPTY; // 清除敌人位置标记
                }
                if (player->hp <= 0) {
        ...
        if (currentFloorState.map[new_x][new_y] == UP_STAIRS && currentFloor == 1) {
            ...
            floor1State.enemy2 = currentFloorState.enemy2;
        ...
        if (currentFloorState.map[new_x][new_y] == DOWN_STAIRS && currentFloor == 2) {
            ...
            floor2State.enemy2 = currentFloorState.enemy2;
...
void trigger_dialogue(Character *player, int map[HEIGHT][WIDTH], PIMAGE npc1Image, PIMAGE playerImage, PIMAGE playerWithSwordImage,
                      ...
                      PIMAGE wallImage, PIMAGE doorImage, PIMAGE keyImage, PIMAGE heartImage, PIMAGE upStairsImage) {
    ...
    for (int i = 0; i < num_dialogues; i++) {
        cleardevice();
        draw_map(map, *player, (Character){0,0,0,0}, (Character){0,0,0,0}, playerImage, playerWithSwordImage, swordImage, orangeImage, enemyImage, enemy2Image, goldImage, wallImage, doorImage, keyImage, npc1Image, upStairsImage, NULL, false);
