...
void move_player(Character *player, int dx, int dy, FloorState &floor1State, FloorState &floor2State, int &currentFloor,
    ...
    if (currentFloorState.map[new_x][new_y] != WALL) {
        ...
        if (currentFloorState.map[new_x][new_y] == NPC_1) {
            trigger_dialogue(player, currentFloorState.map, npc1Image, playerImage, playerWithSwordImage, swordImage, orangeImage, enemyImage, enemy2Image, goldImage, wallImage, doorImage, keyImage, heartImage, upStairsImage, downStairsImage, princessImage);
        }
            ...
            if (enemy->hp > 0) { // 只有在敌人活着的时候才进行战斗
                while (player->hp > 0 && enemy->hp > 0) {
                    ...
                    for (int i = 0; i < 5; i++) {
                        cleardevice();
                        Sleep(50);
                        draw_map(currentFloorState.map, *player, currentFloorState.enemy, (Character) { 0, 0, 0, 0 }, playerImage, playerWithSwordImage, swordImage, orangeImage, enemyImage, enemy2Image, goldImage, wallImage, doorImage, keyImage, npc1Image, upStairsImage, downStairsImage, princessImage, true);
        ...
        // 检测是否碰到公主
        if (currentFloorState.map[new_x][new_y] == PRINCESS) {
            PIMAGE endImage = newimage();
            getimage(endImage, "images/TYFP.png");
...
void trigger_dialogue(Character *player, int map[HEIGHT][WIDTH], PIMAGE npc1Image, PIMAGE playerImage, PIMAGE playerWithSwordImage,
    ...
    for (int i = 0; i < num_dialogues; i++) {
        cleardevice();
        draw_map(map, *player, (Character){0,0,0,0}, (Character){0,0,0,0}, playerImage, playerWithSwordImage, swordImage, orangeImage, enemyImage, enemy2Image, goldImage, wallImage, doorImage, keyImage, npc1Image, upStairsImage, downStairsImage, princessImage, false);
